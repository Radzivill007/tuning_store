 ...
 */
class My_Filters extends WC_Widget {
	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type ) {
		...
		echo '<ul class="catalog__filter-list">';
		...
		foreach ( $terms as $term ) {
			...
			if ( $count > 0 || $option_is_set ) {
				...
				$term_html = '<a rel="nofollow" href="' . esc_url( $link ) . '"><div class="checkbox">
					<input type="checkbox" id="radio' . $radio_key . '"' . ($option_is_set ? 'checked' : '') . '>
					<label for="radio' . $radio_key . '"></label>
				</div>' . esc_html( $term->name ) . '</a>';
			} else {
				$link      = false;
...
function register_my_filters_widget() {
	register_widget( 'My_Filters' );
}
add_action( 'widgets_init', 'register_my_filters_widget' );